...
const jpLanguageMap: LanguageMap = {
  ...
  hintForSavePathOfLogOfFailedConvert: '失敗に関するログはlogsフォルダに保存されました',
  ...
  hintForEmptyItemInAssGeneration: '字幕ファイル生成のパラメータは空欄できません',
  hintForInvalidRegex: (index: number, regex: string) => `${index + 1}行目に無効な正規表現が存在しています：${regex}'`,
  hintForInvalidConfigForCommentsConvert: '無効なコメント変換設定によって変換できませんでした',
  hintForSucceededDownloadAndConvert: (title: string, total: number) => `コメントを保存して変換しました：${title}，実際で保存したのは${total}個でした`,
  hintForFailedConvertWithSucceededDownload: (title: string, total: number) => `コメントの保存は成功で変換は失敗しました：${title}，実際で保存したのは${total}個でした，失敗に関するログはlogsフォルダに保存されました`,
